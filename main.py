...
class Processor():
    ...
    def load_arg(self, batch_size, split):
        Feeder = import_class(self.arg.feeder)
        self.data_loader = dict()
        self.arg.batch_size = batch_size
        self.arg.test_batch_size = batch_size
        if split == 'train':
            m = Feeder(**self.arg.train_feeder_args)
        else:
            m = Feeder(**self.arg.test_feeder_args)
...
def str2bool(v):
    """
    Convert parsed argument into boolean values
    """
